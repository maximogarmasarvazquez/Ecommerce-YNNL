    ...
    <Link href={`/product/${product.attributes.slug}`}
        ...
        <div className="absolute flex items-center justify-between gap-3 px-2 z-[1] top-4">
            ...
            className="px-2 py-1 text-xs text-white bg-black rounded-full
             dark:bg-white dark:text-black w-fit">{product.attributes.estilo}</p>
            <p 
            className={`px-2 py-1 text-xs text-white ${getColorClass(product.attributes.color)}  rounded-full
             dark:bg-white dark:text-black w-fit`}>{product.attributes.color}</p>
        </div>
        <Carousel
        opts={{
            align: "start",
            ...
            <CarouselContent>
                {product.attributes.images.data.map((image) => (
                    <CarouselItem key={image.id} className="group"> 
                        ...
                        />
                        <div className="absolute w-full px-6 transition 
                        duration-200 opacity-0 group-hover:opacity-100 bottom-5">
                            <div className="flex justify-center gap-x-6">
                             <IconButton onClick={() => router.push(`/product/${product.attributes.slug}`) }
                                icon={
                                <Expand size={20} className='text-gray-600'/>}/>
                              <IconButton onClick={() => console.log('ssadas') }
                                icon={
